...
- (void)setTitleWithSelectedIndexPaths:(NSArray *)indexPaths
{
    ...
    if (photosSelected && videoSelected)
        format = NSLocalizedString(@"Selected %ld Items", nil);
    
    else if (photosSelected)
        format = (indexPaths.count > 1) ? NSLocalizedString(@"Selected %ld photos", nil) : NSLocalizedString(@"Selected %ld photo ", nil);
    
    else if (videoSelected)
        format = (indexPaths.count > 1) ? NSLocalizedString(@"Selected %ld videos", nil) : NSLocalizedString(@"Selected %ld video ", nil);
...
- (void)showNotAllowed
{
    ...
    title.text              = NSLocalizedString(@"This application can not use your photo or video.", nil);
    title.font              = [UIFont boldSystemFontOfSize:17.0];
    title.textColor         = [UIColor colorWithRed:129.0/255.0 green:136.0/255.0 blue:148.0/255.0 alpha:1];
    title.textAlignment     = NSTextAlignmentCenter;
    title.numberOfLines     = 5;
    
    message.text            = NSLocalizedString(@"You can enable access to the privacy settings on.", nil);
...
- (void)showNoAssets
{
    ...
    message.text            = NSLocalizedString(@"You can use iTunes to photos and video \ n sync to the iPhone.", nil);
